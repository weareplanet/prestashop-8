...
class WeArePlanetHelper
{
    ...
    {
        $url = Configuration::getGlobalValue(WeArePlanetBasemodule::CK_BASE_URL);
    ...
    public static function getApiClient()
    {
        if (self::$apiClient === null) {
            $userId = Configuration::getGlobalValue(WeArePlanetBasemodule::CK_USER_ID);
            $userKey = Configuration::getGlobalValue(WeArePlanetBasemodule::CK_APP_KEY);
            if (! empty($userId) && ! empty($userKey)) {
                self::$apiClient = new \WeArePlanet\Sdk\ApiClient($userId, $userKey);
                ...
                throw new WeArePlanetExceptionIncompleteconfig();
            }
        }
    ...
    public static function lockByTransactionId($spaceId, $transactionId)
    {
        Db::getInstance()->getLink()->query(
            'SELECT * FROM ' . _DB_PREFIX_ . 'pln_transaction_info WHERE transaction_id = "' .
            (int) $transactionId . '" AND space_id = "' . (int) $spaceId . '" FOR UPDATE;'
        );

        Db::getInstance()->getLink()->query(
            'UPDATE ' . _DB_PREFIX_ . 'pln_transaction_info SET locked_at = "' . pSQL(date('Y-m-d H:i:s')) .
    ...
    {
        /* @var WeArePlanetProviderCurrency $currency_provider */
        $currencyProvider = WeArePlanetProviderCurrency::instance();
     ...
     *
     * @param \WeArePlanet\Sdk\Model\LineItem[] $lineItems
     * @return float
     */
    ...
    public static function cleanupLineItems(array &$lineItems, $expectedSum, $currencyCode)
    {
        ...
        if ($diff != 0) {
            if ((int) Configuration::getGlobalValue(WeArePlanetBasemodule::CK_LINE_ITEM_CONSISTENCY)) {
                throw new WeArePlanetExceptionInvalidtransactionamount($effectiveSum, $roundedExpected);
            } else {
                ...
                $lineItem->setAmountIncludingTax($diffAmount)->setType(($diff > 0) ? \WeArePlanet\Sdk\Model\LineItemType::FEE : \WeArePlanet\Sdk\Model\LineItemType::DISCOUNT);
     ...
     *
     * @param \WeArePlanet\Sdk\Model\LineItem[] $lineItems
     * @param \WeArePlanet\Sdk\Model\LineItemReduction[] $reductions
     * @return float
     */
    ...
    public static function updateCartMeta(Cart $cart, $key, $value)
    {
        Db::getInstance()->execute(
            'INSERT INTO ' . _DB_PREFIX_ . 'pln_cart_meta (cart_id, meta_key, meta_value) VALUES ("' . (int) $cart->id .
            '", "' . pSQL($key) . '", "' . pSQL(WeArePlanetTools::base64Encode(serialize($value))) .
            '") ON DUPLICATE KEY UPDATE meta_value = "' .
            pSQL(WeArePlanetTools::base64Encode(serialize($value))) . '";'
        );
    }
    ...
    public static function getCartMeta(Cart $cart, $key)
    {
        $value = Db::getInstance()->getValue(
            'SELECT meta_value FROM ' . _DB_PREFIX_ . 'pln_cart_meta WHERE cart_id = "' . (int) $cart->id .
            '" AND meta_key = "' . pSQL($key) . '";',
            false
        );
        if ($value !== false) {
            $decoded = WeArePlanetTools::base64Decode($value, true);
            if ($decoded === false) {
                $decoded = $value;
            }
    ...
    {
        Db::getInstance()->execute(
            'DELETE FROM ' . _DB_PREFIX_ . 'pln_cart_meta WHERE cart_id = "' . (int) $cart->id . '" AND meta_key = "' .
            pSQL($key) . '";',
            false
        );
    }
    ...
    {
        return Module::getInstanceByName('weareplanet');
    }
    ...
    {
        Db::getInstance()->execute(
            'DELETE FROM ' . _DB_PREFIX_ . 'pln_order_meta WHERE order_id = "' . (int) $order->id . '" AND meta_key = "' .
            pSQL($key) . '";',
            false
        );
    }
    ...
    public static function getOrderEmails(Order $order)
    {
        ...
        if ($value !== false) {
            return unserialize(WeArePlanetTools::base64Decode($value));
        }
        return array();
    }
    ...
    {
        return WeArePlanetTools::hashHmac('sha256', $order->id, $order->secure_key, false);
    }

    /**
     * Sorts an array of WeArePlanetModelMethodconfiguration by their sort order
     *
     * @param WeArePlanetModelMethodconfiguration[] $configurations
     *
     * @return array
     */
     ...
     * @return string
     */
    public static function getTransactionState(WeArePlanetModelTransactioninfo $info)
    {
     ...
     * @return string
     */
    public static function getTransactionUrl(WeArePlanetModelTransactioninfo $info)
    {
     ...
     * @return string
     */
    public static function getRefundUrl(WeArePlanetModelRefundjob $refundJob)
    {
     ...
     * @return string
     */
    public static function getCompletionUrl(WeArePlanetModelCompletionjob $completion)
    {
     ...
     * @return string
     */
    public static function getVoidUrl(WeArePlanetModelVoidjob $void)
    {
     ...
     */
    public static function getGroupedChargeAttemptLabels(WeArePlanetModelTransactioninfo $info)
    {
        try {
            $labelDescriptionProvider = WeArePlanetProviderLabeldescription::instance();
            $labelDescriptionGroupProvider = WeArePlanetProviderLabeldescription::instance();

            $labelsByGroupId = array();
            foreach ($info->getLabels() as $descriptorId => $value) {
                $descriptor = $labelDescriptionProvider->find($descriptorId);
                if ($descriptor &&
                    $descriptor->getCategory() == \WeArePlanet\Sdk\Model\LabelDescriptorCategory::HUMAN) {
                    $labelsByGroupId[$descriptor->getGroup()][] = array(
                        'descriptor' => $descriptor,
                        'translatedName' => WeArePlanetHelper::translate($descriptor->getName()),
                        'value' => $value
                    );
                }
            }
            ...
            foreach ($labelsByGroupId as $groupId => $labels) {
                ...
                if ($group) {
                    ...
                    $labelsByGroup[] = array(
                        'group' => $group,
                        'id' => $group->getId(),
                        'translatedTitle' => WeArePlanetHelper::translate($group->getName()),
                        'labels' => $labels
                    );
                }
            }
     ...
     * If the order id is not associated with a WeArePlanet transaciton it returns null
     *
     * @return WeArePlanetModelTransactioninfo | null
     */
    public static function getTransactionInfoForOrder($order)
    {
        if (! $order->module == 'weareplanet') {
            return null;
        }
        $searchId = $order->id;

        $mainOrder = self::getOrderMeta($order, 'weArePlanetMainOrderId');
        if ($mainOrder !== null) {
            $searchId = $mainOrder;
        }
        $info = WeArePlanetModelTransactioninfo::loadByOrderId($searchId);
        if ($info->getId() == null) {
            return null;
        }
        return $info;
