 ...
 */
class WeArePlanetBackendDefaultstrategy implements WeArePlanetBackendIstrategy
{
    ...
    public function validateAndParseData(Order $order, array $postData)
    {
        ...
        throw new Exception(
            WeArePlanetHelper::getModuleInstance()->l('The refund type is not supported.', 'defaultstrategy')
        );
    }
    ...
    public function simplifiedRefund(array $postData)
    {
        ...
        if ($amount >= 0) {
            return array(
                ...
                'weArePlanetOffline' => isset($postData['weareplanet_offline'])
            );
        } else {
            if (! empty($quantity)) {
                throw new Exception(
    ...
    private function validateDataCancelProductType(Order $order, array $postData)
    {
        if (! isset($postData['id_order_detail']) && ! isset($postData['id_customization'])) {
            throw new Exception(
                WeArePlanetHelper::getModuleInstance()->l('You must select a product.', 'defaultstrategy')
            );
        } elseif (! isset($postData['cancelQuantity']) && ! isset($postData['cancelCustomizationQuantity'])) {
            throw new Exception(
                WeArePlanetHelper::getModuleInstance()->l('You must enter a quantity.', 'defaultstrategy')
            );
        } else {
        ...
        if ($reductionPercent > 0) {
            if ($reductionProduct > 0) {
                ...
                foreach ($usedTaxes as $id => $values) {
                    ...
                    $totalAmount = WeArePlanetHelper::roundAmount(
        ...
        if ((float) $reductionAmount > 0) {
            if ($reductionProduct > 0) {
                ...
                foreach ($usedTaxes as $id => $values) {
                    ...
                    $totalAmount = WeArePlanetHelper::roundAmount($amount * - 1, $currencyCode);
                    if ($totalAmount == 0) {
                        continue;
                    }
                    $discountUniqueIds[] = $uniqueIdBase . '-' . $id;
    ...
    private function applyRefundPartialRefundType(Order $order, array $parsedData)
    {
        ...
        if ($parsedData['generateDiscountRefund']) {
            ...
            $cart_rule->description = sprintf(
                WeArePlanetHelper::getModuleInstance()->l('Credit slip for order #%d', 'defaultstrategy'),
                $order->id
            );
    ...
    public function getWeArePlanetRefundType(array $parsedData)
    {
        if ($parsedData['weArePlanetOffline']) {
            return \WeArePlanet\Sdk\Model\RefundType::MERCHANT_INITIATED_OFFLINE;
        }
        return \WeArePlanet\Sdk\Model\RefundType::MERCHANT_INITIATED_ONLINE;
    }
    ...
    {
        return WeArePlanetVersionadapter::isVoucherOnlyWeArePlanet($postData);
    }
    ...
    public function processVoucherDeleteRequest(Order $order, array $data)
    {
        ...
        if (Validate::isLoadedObject($order_cart_rule) && $order_cart_rule->id_order == $order->id) {
            ...
            throw new Exception(
                WeArePlanetHelper::getModuleInstance()->l('You cannot edit this cart rule.', 'defaultstrategy')
            );
        }
    }
