 ...
 */
class WeArePlanetServiceLineitem extends WeArePlanetServiceAbstract
{
    ...
    {
        $currencyCode = WeArePlanetHelper::convertCurrencyIdToCode($cart->id_currency);
        $items = array();
        ...
        foreach ($summary['products'] as $productItem) {
            ...
            foreach (array_keys($psTaxes) as $id) {
                $psTax = new Tax($id);
                $tax = new \WeArePlanet\Sdk\Model\TaxCreate();
            ...
            $item->setTaxes($taxes);
            $item->setType(\WeArePlanet\Sdk\Model\LineItemType::PRODUCT);
            if ($productItem['id_product'] == Configuration::get(WeArePlanetBasemodule::CK_FEE_ITEM) ||
                $productItem['id_product'] == Configuration::get(WeArePlanetBasemodule::CK_SURCHARGE_ITEM)) {
                $item->setType(\WeArePlanet\Sdk\Model\LineItemType::FEE);
        ...
        if ($shippingCosts > 0) {
            ...
            $item->setQuantity(1);
            $item->setShippingRequired(false);
            $item->setSku(WeArePlanetHelper::getModuleInstance()->l('Shipping', 'lineitem'));
            $name = "";
            ...
            $name = empty($name) ? WeArePlanetHelper::getModuleInstance()->l('Shipping', 'lineitem') : $name;
            $name = $this->trimStringByLength($name, 150);
            $item->setName($name);
            if (! $taxCalculatorFound) {
                $taxRate = 0;
                $taxName = WeArePlanetHelper::getModuleInstance()->l('Tax', 'lineitem');
            ...
            $item->setType(\WeArePlanet\Sdk\Model\LineItemType::SHIPPING);
        ...
        if ($wrappingCosts > 0) {
            ...
            $name = WeArePlanetHelper::getModuleInstance()->l('Wrapping Fee', 'lineitem');
            $name = $this->trimStringByLength($name, 150);
            $item->setName($name);
            $item->setQuantity(1);
            $item->setShippingRequired(false);
        ...
        if (count($summary['discounts']) > 0) {
            ...
            foreach ($summary['discounts'] as $discount) {
                $nameBase = empty($discount['description']) ? WeArePlanetHelper::getModuleInstance()->l(
                    'Discount',
                    'lineitem'
        ...
        if ($discountOnly && $shippingItem != null) {
            $itemFreeShipping = new \WeArePlanet\Sdk\Model\LineItemCreate();
            $name = empty($discountOnly['name']) ? WeArePlanetHelper::getModuleInstance()->l(
                'Shipping Discount',
                'lineitem'
            ) : $discountOnly['name'];
            $name = $this->trimStringByLength($name, 150);
    ...
    private function extractTaxName($psTax, $cart)
    {
        ...
        if (empty($translated) || strlen($translated) < 2) {
            ...
            return WeArePlanetHelper::getModuleInstance()->l('Tax', 'lineitem');
        } else {
            return $translated;
        }
    }
    ...
    public function getItemsFromOrders(array $orders)
    {
        ...
        $cleaned = WeArePlanetHelper::cleanupLineItems(
            $items,
            $orderTotal,
            WeArePlanetHelper::convertCurrencyIdToCode($order->id_currency)
        );
        return $cleaned;
    }
    ...
    protected function getItemsFromOrdersInner(array $orders, $orderTotal)
    {
        ...
        foreach ($orders as $order) {
            /*@var Order $order */
            $currencyCode = WeArePlanetHelper::convertCurrencyIdToCode($order->id_currency);
            ...
            foreach ($order->getProducts() as $orderItem) {
                ...
                $name = empty($orderItem['product_name']) ? WeArePlanetHelper::getModuleInstance()->l(
                    'Product',
                    'lineitem'
                ...
                if ($orderItem['product_id'] == Configuration::get(WeArePlanetBasemodule::CK_FEE_ITEM) ||
                    $orderItem['product_id'] == Configuration::get(WeArePlanetBasemodule::CK_SURCHARGE_ITEM)) {
                    $item->setType(\WeArePlanet\Sdk\Model\LineItemType::FEE);
                    $item->setShippingRequired(false);
                }
            ...
            foreach ($order->getCartRules() as $orderCartRule) {
                ...
                $nameBase = empty($orderCartRule['name']) ? WeArePlanetHelper::getModuleInstance()->l(
                    'Discount',
                    'lineitem'
                ) : $orderCartRule['name'];
                $discountItems = $this->getDiscountItems(
        ...
        if ($cartRule->gift_product != 0) {
            foreach ($existingLineItems as $exisitingLineItem) {
                if ($exisitingLineItem->getUniqueId() ==
                    ...
                    $nameBase = $this->trimStringByLength($nameBase, 150);
                    $item->setName($nameBase);
                    $item->setQuantity(1);
                    $item->setShippingRequired(false);
                    $item->setSku($skuBase . '-gift');
                    $item->setTaxes($exisitingLineItem->getTaxes());
                    $item->setType(\WeArePlanet\Sdk\Model\LineItemType::DISCOUNT);
        ...
        if ($reductionPercent > 0) {
            if ($reductionProduct > 0) {
                ...
                if ($reductionProduct == - 2) {
                    $selectedProducts = WeArePlanetCartruleaccessor::checkProductRestrictionsStatic(
                        $cartRule,
                        new Cart($cartIdUsed)
                    );
     ...
     *
     * @param \WeArePlanet\Sdk\Model\LineItemCreate $lineItem
     * @return \WeArePlanet\Sdk\Model\LineItemCreate
     */
    protected function cleanLineItem(\WeArePlanet\Sdk\Model\LineItemCreate $lineItem)
    {
     ...
     * @return string
     */
    protected function trimStringByLength(string $string, int $lentgh): string
    {
        return $this->fixLength($string, $lentgh);
    }
}
