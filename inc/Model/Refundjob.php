...
class WeArePlanetModelRefundjob extends ObjectModel
{
    ...
    {
        $this->refund_parameters = WeArePlanetTools::base64Encode(serialize($params));
    }

    public function getRefundParameters()
    {
        $decoded = WeArePlanetTools::base64Decode($this->refund_parameters, true);
        if ($decoded === false) {
    ...
    {
        $decoded = WeArePlanetTools::base64Decode($this->failure_reason, true);
        if ($decoded === false) {
    ...
    {
        $this->failure_reason = WeArePlanetTools::base64Encode(serialize($failureReason));
    }
    ...
    public static function isRefundRunningForTransaction($spaceId, $transactionId)
    {
        $result = DB::getInstance()->getValue(
            'SELECT id_refund_job FROM ' . _DB_PREFIX_ . 'pln_refund_job WHERE space_id = "' . (int) $spaceId .
    ...
    public static function loadNotSentJobIds()
    {
        ...
        $result = DB::getInstance()->query(
            'SELECT id_refund_job FROM ' . _DB_PREFIX_ . 'pln_refund_job WHERE state = "' . pSQL(self::STATE_CREATED) .
    ...
    public static function loadNotAppliedJobIds()
    {
        $time = new DateTime();
        $result = DB::getInstance()->query(
            'SELECT id_refund_job FROM ' . _DB_PREFIX_ . 'pln_refund_job WHERE state = "' . pSQL(self::STATE_APPLY) .
            '" AND date_upd < "' . pSQL($time->format('Y-m-d H:i:s')) . '"',
            false
        );
        $ids = array();
