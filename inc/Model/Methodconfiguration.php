...
class WeArePlanetModelMethodconfiguration extends ObjectModel
{
    ...
    public $fee_base = WeArePlanetBasemodule::TOTAL_MODE_BOTH_INC;
    ...
    public static $definition = array(
        'table' => 'pln_method_configuration',
    ...
    {
        $this->title = WeArePlanetTools::base64Encode(serialize($title));
    }

    public function getTitle()
    {
        $decoded = WeArePlanetTools::base64Decode($this->title, true);
        if ($decoded === false) {
            $decoded = $this->title;
        }
    ...
    {
        $this->description = WeArePlanetTools::base64Encode(serialize($description));
    }

    public function getDescription()
    {
        $decoded = WeArePlanetTools::base64Decode($this->description, true);
        if ($decoded === false) {
     ...
     * @param int $id
     * @param int $shopId
     * @return WeArePlanetModelMethodconfiguration | false
     */
    public static function loadByIdWithChecks($id, $shopId)
    {
        ...
        $collection = new PrestaShopCollection('WeArePlanetModelMethodconfiguration');
     ...
     * @return WeArePlanetModelMethodconfiguration[]
     */
    ...
    public static function loadByConfigurationAndShop($spaceId, $configurationId, $shopId)
    {
        ...
        if ($result === false) {
            $result = new WeArePlanetModelMethodconfiguration();
        }
        return $result;
    }
