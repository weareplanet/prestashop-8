...
class WeArePlanetDocumentsModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if ($type = Tools::getValue('type')) {
            switch ($type) {
                case 'invoice':
                    if ((bool) Configuration::get(WeArePlanetBasemodule::CK_INVOICE)) {
                        $this->processWeArePlanetInvoice($order);
                    }
                    break;
                case 'packingSlip':
                    if ((bool) Configuration::get(WeArePlanetBasemodule::CK_PACKING_SLIP)) {
                        $this->processWeArePlanetPackingSlip($order);
                    }
                    break;
            }
        }
    ...
    private function processWeArePlanetInvoice($order)
    {
        try {
    ...
    private function processWeArePlanetPackingSlip($order)
    {
        try {
